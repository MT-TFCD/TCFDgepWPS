...
class GeneDc(Gene):
    ...
    def __init__(self, pset, head_length, rnc_gen, rnc_array_length):
        ...
        self.pset=pset
        if head_length>1:
            max_arity=pset.max_arity
        else:
            max_arity = 0
            
        self._head_length=head_length
        self.t = head_length * (max_arity - 1) + 1
        self.d = self.t
        if self._head_length==1:
            self.d=5
        # complement it with a Dc domain
        self._rnc_gen = rnc_gen
        dc = generate_dc(rnc_array_length, dc_length=self.d)
    ...
    def dc_length(self):
        ...
        """
        return self.d   #or self.d  OR self.head_length * (self.max_arity - 1) + 1
    ...
    def tail_length(self):
        ...
        (len(self) - self.head_length) // 2
        """
        return self.t   #or self.t  OR self.head_length * (self.max_arity - 1) + 1 
...
class Chromosome_goody(list):
    ...
    def __init__(self, gene_gens, n_genes, linker=None):
        """
        Initialize an individual in GEP.
        ...
        .. note:
            ...
            *linker* parameter is the default value ``None``, then for a monogenic chromosome, no linking is applied,
            while for a multigenic chromosome, the ``tuple`` function is used.
        """
        list.__init__(self, (gene_gens[_]() for _ in range(n_genes)))
        self._linker = linker
